...
- **Graceful shutdown + bounded server** — SIGTERM drains in-flight requests before exit, so rolling updates of the tool itself drop nothing. The `http.Server` carries read/write/header timeouts (slow clients get bounded like slow upstreams), and every grace period strictly exceeds the handler timeout it wraps, so the layers never race.
